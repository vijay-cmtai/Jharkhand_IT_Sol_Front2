...
const industryInfo = {
  ...
  keySolutions: [
    ...
    },
    {
      ...
      icon: Cpu,
    }, // Changed icon to Cpu for variety, was Smartphone
    {
  ...
  // Added for potential sidebar content if needed, but the screenshot already shows "Core Technologies"
  whyPartnerPoints: [
...
const EcommerceDetailPage: React.FC = () => {
  ...
  return (
    <div className="bg-slate-950 text-slate-300 min-h-screen antialiased flex flex-col">
      ...
      <main className="flex-grow">
        ...
        >
          <div className="absolute inset-0 z-0">
            ...
            {/* Adjusted gradient to purple for e-commerce feel */}
          </div>
              ...
              <div className="bg-slate-800/60 p-6 rounded-xl border border-slate-700/50 shadow-lg backdrop-blur-sm hover:border-slate-600/70 transition-colors">
                ...
                <ul className="space-y-3 text-sm text-slate-300/95">
                  {industryInfo.whyPartnerPoints.map(
                    (
                      point,
                      index // Using whyPartnerPoints from industryInfo
                    ) => (
              ...
              <div className="bg-slate-800/60 p-6 rounded-xl border border-slate-700/50 shadow-lg backdrop-blur-sm hover:border-slate-600/70 transition-colors">
                ...
                <div className="flex flex-wrap gap-2.5">
                  ...
                  {industryInfo.relevantTechnologies.map((tech) => (
                    <span
                      key={tech}
                      className="bg-slate-700/80 text-cyan-300/90 px-3.5 py-2 rounded-lg text-xs font-medium border border-slate-600/70 hover:bg-slate-600/80 hover:text-cyan-200 hover:border-slate-500 transition-all duration-200 cursor-default" // Increased padding, rounded-lg
                    >
                      {tech}
                    </span>
                  ))}
      ...
      {/* <Footer /> */} {/* Keep commented if not needed */}
    </div>
  );
};
