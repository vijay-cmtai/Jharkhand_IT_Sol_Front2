...
// Define your backend base URL (might still be needed for other API calls or if some images are relative)
...
interface ApiPostData {
  ...
  excerpt?: string;
  content: string;
  author: { name: string } | string; // Assuming your API returns a simple string for author now
  category: { name: string } | string; // Assuming your API returns a simple string for category now
  imageUrl: string; // CORRECTED: This is the field from your API for the image URL
  createdAt: string; // Assuming this maps to publishDate or createdAt for sorting/display
  publishDate?: string; // Added based on your sample
  commentsCount?: number; // You might need to derive this or your API might send it
  tags?: string[];
  status?: string;
}

// Interface for blog posts used within the component
interface BlogPost {
  id: string;
  title: string;
  excerpt: string;
  ...
  date: string;
  category: string;
  image: string; // This will be the final, usable image URL
  comments: number;
  slug: string;
}
...
const filterCategories = [
  ...
  "UX/UI Design",
  "E-commerce",
  "Programming", // Added from your sample
];
...
const getCategoryIcon = (categoryName: string): React.ReactNode => {
  ...
  switch (categoryName) {
    ...
    case "Programming": // Grouping Programming with Web Development icon for now
    ...
    const fetchBlogPosts = async () => {
      ...
      try {
        ...
        const transformedPosts: BlogPost[] = data.map((post) => {
          const imageUrl = post.imageUrl; // CORRECTED: Use post.imageUrl
          // No need to prepend BASE_URL if imageUrl is already an absolute URL (like Cloudinary)
          ...
          return {
            ...
            author:
              typeof post.author === "string"
                ? post.author
                : (post.author as { name: string }).name, // Handle both string and object for author
            date: new Date(
              post.publishDate || post.createdAt
            ).toLocaleDateString("en-US", {
              // Prefer publishDate if available
              year: "numeric",
              month: "long",
              day: "numeric",
            }),
            category:
              typeof post.category === "string"
                ? post.category
                : (post.category as { name: string }).name, // Handle both string and object for category
            image: imageUrl,
            comments: post.commentsCount || 0, // Assuming 0 if not provided
            slug: post.slug,
          };
        });
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-slate-900 to-black text-gray-300">
      <main className="flex-grow">
        {/* Hero Section */}
        <motion.section
          className={`relative bg-gradient-to-br from-blue-800/70 via-emerald-700/60 to-slate-900 py-20 md:py-28 overflow-hidden`}
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <motion.div
                ...
                variants={itemVariants}
              >
                {/* Dynamically generate filter categories if needed, or keep existing static list and add "Programming" */}
                ...
                {paginatedPosts.length > 0 ? (
                  ...
                  >
                    ...
                    {paginatedPosts.map((post) => (
                      <motion.div
                        layout
                        key={post.slug} // Make sure slug is unique
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {post.image ? (
                              <img
                                src={post.image} // This should now be the correct Cloudinary URL
                                alt={post.title}
