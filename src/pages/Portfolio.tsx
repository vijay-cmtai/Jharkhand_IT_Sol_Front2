...
interface PortfolioItem {
  ...
  image: string;
  excerpt: string;
  slug: string; // Internal link if no projectUrl
  projectUrl?: string; // External project link
}
...
const Portfolio: React.FC = () => {
    ...
    const fetchPortfolioItems = async () => {
      ...
      try {
        ...
        const mappedData: PortfolioItem[] = data.map((item: any) => ({
          ...
          slug: `/portfolio/${item._id}`, // Fallback internal slug
          projectUrl: item.projectLink, // External project URL
        }));
  ...
  const cardVariants = {
    ...
    }, // Updated hover shadow to use amber
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-slate-900 to-black text-gray-300">
      <main className="flex-grow">
        {/* --- Hero Section (Filters, Title) --- */}
        <motion.section
          className={`relative bg-gradient-to-br from-${jisBrown}/70 via-${jisYellow}/60 to-slate-900 py-20 md:py-28 overflow-hidden`}
                  ...
                  <motion.button
                    ...
                    className={cn(
                      ...
                      activeFilter === category
                        ...
                        : `bg-slate-700/50 text-gray-200 hover:bg-slate-600/70 ring-slate-600 hover:ring-slate-500`
                    )}
        ...
        >
          <div className="container mx-auto px-4">
            <AnimatePresence mode="popLayout">
              ...
              >
                {itemsToShow.map((project) => (
                  <motion.div // This is the main card div, NOT a link.
                    layout
                    key={project.id}
                    ...
                    animate="animate"
                    exit="exit"
                    whileHover="hover" // Uses hover from cardVariants
                  >
                    {/* Card Content - Not Clickable Directly */}
                    <div className="flex flex-col flex-grow">
                      ...
                      >
                        <img
                          src={project.image}
                          alt={project.title}
                          className="w-full h-full object-cover transition-transform duration-500 ease-in-out group-hover:scale-105" // group-hover for image scale
                          loading="lazy"
                        />
                        <div
                      ...
                      <div className="p-5 md:p-6 flex flex-col flex-grow">
                        ...
                        <h3
                          className={`text-lg md:text-xl font-bold text-white mb-2 group-hover:text-amber-300 transition-colors duration-300 line-clamp-2`} // group-hover for title color
                        >
                          {project.title}
                        </h3>
                        ...
                        <div className="mt-auto self-start">
                          {project.projectUrl ? (
                            <a
                              ...
                              className={`inline-flex items-center text-amber-400/80 hover:text-amber-300 font-medium text-sm transition-colors duration-300 hover:underline cursor-pointer`} // Added cursor-pointer
                            >
                              Visit Project{" "}
                            ...
                            <Link
                              to={project.slug}
                              className={`inline-flex items-center text-amber-400/80 hover:text-amber-300 font-medium text-sm transition-colors duration-300 cursor-pointer`} // Added cursor-pointer
                            >
                              View Details{" "}
            ...
            {/* --- Load More and No Data Messages --- */}
            ...
            {/* --- End Load More and No Data --- */}
          </div>
        </motion.section>
        {/* --- End Portfolio Grid Section --- */}
